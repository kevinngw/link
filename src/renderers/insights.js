...
export function createInsightsRenderers(deps) {
  ...
  function renderSystemSummary(insightsItems) {
    const metrics = computeSystemSummaryMetrics({
      insightsItems,
      classifyHeadwayHealth,
      computeLineHeadways,
      copyValue,
    ...
    return `
      <article class="panel-card panel-card-wide system-summary-card">
        <header class="panel-header"><div class="system-summary-header"><div class="line-title"><span class="line-token" data-line-token-type="${getLineTokenType(getActiveSystemMeta().label)}" style="--line-color:var(--accent-strong);">${getLineToken(getActiveSystemMeta().label)}</span><div class="line-title-copy"><h2>${getActiveSystemMeta().label} ${copyValue('summaryTitle')}</h2><p>${copyValue('systemLinesUpdated', metrics.totalLines, formatCurrentTime())}</p></div></div></div></header>
        ...
        <div class="system-priority"><div class="insight-exceptions-header"><p class="headway-chart-title">${copyValue('recommendedNext')}</p><p class="headway-chart-copy">${copyValue('recommendedNextNote')}</p></div><div class="system-priority-list">${(metrics.priorityLines.length ? metrics.priorityLines : metrics.rankedLines.slice(0, 1)).map(({ line, worstGap, severeLateCount, alertCount, attentionReasons }) => `<div class="system-priority-item insights-clickable" data-insights-line-id="${line.id}" data-insights-type="ranking" role="button" tabindex="0"><div class="line-title"><span class="line-token" data-line-token-type="${getLineTokenType(line.name)}" style="--line-color:${line.color};">${getLineToken(line.name)}</span><div class="line-title-copy"><p class="headway-chart-title">${line.name}</p><p class="headway-chart-copy">${worstGap ? copyValue('gapMinSummary', worstGap) : copyValue('noMajorSpacingIssue')}${severeLateCount ? ` · ${copyValue('severeLateCountText', severeLateCount)}` : ''}${alertCount ? ` · ${copyValue('alertCountText', alertCount)}` : ''}</p>${renderAttentionReasonBadges(attentionReasons)}</div></div></div>`).join('')}</div></div>
        <div class="system-ranking"><div class="insight-exceptions-header"><p class="headway-chart-title">${copyValue('attentionRanking')}</p><p class="headway-chart-copy">${state.error ? copyValue('realtimeDegraded') : copyValue('fromLiveSnapshot')}</p></div><div class="system-ranking-list">${metrics.rankedLines.slice(0, 3).map(({ line, score, worstGap, alertCount, severeLateCount, attentionReasons }) => `<div class="system-ranking-item insights-clickable" data-insights-line-id="${line.id}" data-insights-type="ranking" role="button" tabindex="0"><div class="line-title"><span class="line-token" data-line-token-type="${getLineTokenType(line.name)}" style="--line-color:${line.color};">${getLineToken(line.name)}</span><div class="line-title-copy"><p class="headway-chart-title">${line.name}</p><p class="headway-chart-copy">${copyValue('scoreSummary', score)}${worstGap ? ` · ${copyValue('gapMinSummary', worstGap)}` : ''}${alertCount ? ` · ${copyValue('alertCountText', alertCount)}` : ''}${severeLateCount ? ` · ${copyValue('severeLateCountText', severeLateCount)}` : ''}</p>${renderAttentionReasonBadges(attentionReasons)}</div></div></div>`).join('')}</div></div>
  ...
  function renderInsightsBoard(visibleLines) {
    const systemInsightsItems = buildInsightsItems({
      lines: state.lines,
      layouts: state.layouts,
      vehiclesByLine: state.vehiclesByLine,
      getAlertsForLine,
      decorateItem: ({ line, nb, sb, lineAlerts }) => ({ exceptions: buildLineExceptions(line, nb, sb, lineAlerts) }),
    })
    const vehicleLabel = getVehicleLabel()
    const insightsItems = buildInsightsItems({
      lines: visibleLines,
      layouts: state.layouts,
      vehiclesByLine: state.vehiclesByLine,
      getAlertsForLine,
      ...
      ${insightsItems.map(({ line, layout, vehicles, nb, sb, lineAlerts }) => {
        ...
        return `
          <article class="panel-card panel-card-wide">
            <header class="panel-header line-card-header"><div class="line-title"><span class="line-token" data-line-token-type="${getLineTokenType(line.name)}" style="--line-color:${line.color};">${getLineToken(line.name)}</span><div class="line-title-copy"><h2>${line.name}</h2><p>${copyValue('liveCount', vehicles.length, vehicles.length === 1 ? getVehicleLabel().toLowerCase() : getVehicleLabelPlural().toLowerCase())} · ${getTodayServiceSpan(line)}</p></div></div>${renderServiceReminderChip(line)}</header>
